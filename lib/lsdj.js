...
function LSDSNG(buffer) {
  ...
  this.title = _.map(_.range(8), function(i) {
    return String.fromCharCode(buffer.readUInt8(i))
  }).join('')
  // get version
  this.version = buffer.readUInt8(8)
  // get raw data
  this.data = LSDSNG.unpack(buffer)
  // parse data
      ...
      switch(current) {
        ...
        case 0xE0:
          ...
          if (directive == 0xE0) {        // special byte
            ...
            //throw "Unexpected sequence " + current.toString(16) + " " + directive.toString(16)
          }
          break
